...
class Scraper
  ...
  COURSE_SCHEMA = YAML::load_file(File.join(__dir__, 'courses.yml'))
  ...
  def self.scrape!
    @@mechanize.get('https://bblearn.missouri.edu/webapps/portal/execute/defaultTab') do |login_page|
      login(login_page, CONFIG["pawprint"], CONFIG["password"])
      ...
      COURSE_SCHEMA.each do |course_name, course|
        course["sections"].each do |section_name, section|
          ...
          scrape_section section_url, [course_name, section_name]
        end
      end
    end
  end
  ...
  def self.scrape_section(page_url, dir_array)
    ...
    page.search("//li[substring(@id, 1, 15) = 'contentListItem']").each do |content|
      ...
      scrape_section(File.join(ROOT_URL, url), dir_array + [title]) if url.include? "/webapps/blackboard/content/listContent.jsp"
    end
  end
end
